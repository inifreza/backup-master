                ...
                if(!errAuth){
                    if(resAuth == null || undefined){
                        ...
                        if(resAuth.auth_code == req.body.auth_code){
                            ...
                            hashtag.addData(body, function(err,resData) {
                                console.log(err)
                                if (!err) {
          ...
          if(!errAuth){
            if(!utility.issetVal(resAuth)){
                ...
                if(resAuth.auth_code == req.body.auth_code){
                    ...
                    hashtag.getCountData(bodyCount,function(errResCount,rowsResCount) {
                        ...
                        if (!errResCount) {
                           if (utility.issetVal(rowsResCount)) {
                                ...
                                hashtag.getAll(PreparedData,function(errRes,rowsRes) {
                                    console.log(errRes);
                                    if (!errRes) {
        ...
        if(!errAuth){
          if(!utility.issetVal(resAuth)){
            ...
            if(resAuth.auth_code == req.query.auth_code){
              ...
              hashtag.getById(body, function(errGet,resGet) {
                console.log(errGet);
                ...
                if (!errGet) {
                    if(!utility.issetVal(resGet)){
                        ...
                        hashtag.deleteData(body, function(err,resData) {
                        // caches
                            if (!err) {
        ...
        const result = admin.getAuth(req.body,function(errAuth,resAuth){
            if(!errAuth){
                if(!utility.issetVal(resAuth)){
                    ...
                    hashtag.getById(body, function(err, resById){
                        if(!err){
                            hashtag.updateData(utility.cleanJSON(body), function(err,resData) {
                                if (!err) {
                ...
                if(!errAuth){
                    if(!utility.issetVal(resAuth)){
                        ...
                        if(resAuth.auth_code == req.body.auth_code){
                            ...
                            hashtag.getById(body,function(errRes,resData) {
                                console.log(resData);
                                if (!errRes) {
                                    if (utility.issetVal(resData)) {
                ...
                if(!errAuth){
                    if(!utility.issetVal(resAuth)){
                        ...
                        if(resAuth.auth_code == req.query.auth_code){
                            ...
                            hashtag.getHashtag(null,function(errRes,resData) {
                                console.log(errRes)
                                if (!errRes) {
                                    if (utility.issetVal(resData)) {
                                        let array = [];
                                        for(let idx = 0; idx <  resData.length; idx++) {
                                            array.push(resData[idx].word);
                                        }
