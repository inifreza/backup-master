...
module.exports = {
  getAll : function (req, callback){
    ...
    .modify(qb=>{
      if(utility.issetVal(req.keyword)){
        // qb.whereRaw('alumni_name like ? or alumni_email like ? ', [`%${req.keyword}%`,`%${req.keyword}%`])
      ...
      if(utility.issetVal(req.limit)){
        qb.limit(req.limit)
      }
      if(utility.issetVal(req.start)){
        qb.offset(req.start)
      }
    })
    .groupBy('feedback.id')
  ...
  getCount : function(req, callback){
    ...
    .modify(qb=>{
      if(utility.issetVal(req.keyword)){
        qb.andWhere('user.name', 'LIKE', `%${req.keyword}%`)
        qb.orWhere('user.email', 'LIKE', `%${req.keyword}%`)
      }
      if(utility.issetVal(req.app_version)){
        qb.andWhere('feedback.app_version', 'LIKE', `%${req.app_version}%`)
      }
